...
- **Antigravity lists every effort level of your launch model.** Antigravity has no reasoning-effort control, so Relay now lists a model once per effort level: the model you launch with appears at every level it supports (GPT-6 Sol shows `None` through `Max`), and each favorite appears at three levels — medium and the two above it (e.g. `Medium / High / XHigh`), topped up from below when a model has fewer. Models without adjustable effort appear once. Previously every Antigravity model silently ran at `high`. This applies to Antigravity IDE and the Antigravity app; the agy CLI works differently (next entry).
- **agy CLI: one row per model with a native effort slider.** agy folds entries that differ only by effort into one picker row with its Low / Medium / High / Max slider, so each model now appears once with that slider, plus a separate `XHigh` row when the model supports it (agy's slider has no XHigh position; `None` is left out). agy opens on your launch model at Medium. Relay's agy entries no longer borrow Antigravity's native model slots, which grouped unrelated entries into mislabeled sliders.
- **Claude desktop lists effort levels for non-Claude models.** Claude desktop only shows an effort slider for Claude models, so other models with adjustable effort are listed once per level (e.g. `GPT-6 Sol Low`, `GPT-6 Sol High`): every level for your launch model, medium and the two above it for each favorite. Each entry always runs at its level. Claude models are unchanged.
- **"↻ Refresh models" in every launch picker.** After choosing a provider in `relay-ai claude`, `agy`, `antigravity`, `codex`, `chatgpt`/`codex-app`, `claude-app` or `gemini`, the model list offers a refresh that fetches the provider's latest models and reopens the list — no need to quit and go through `relay-ai providers`.
...
- **Antigravity IDE and app open on the model you launched with.** The IDE remembers your last pick by a hidden model code, and Relay reassigns those codes every launch, so it could reopen on a different model (e.g. you launch GPT-6 Sol and it opens on another favorite). Relay now clears that remembered pick in its own Antigravity profile before each launch. The Antigravity app has the same issue and gets the same fix: Relay removes its remembered pick (one line in `~/.gemini/antigravity/antigravity_state.pbtxt`, a file the app shares with non-Relay use) before launching it.
- **Claude desktop no longer shows "Gateway returned an error" for ChatGPT-login models.** ChatGPT's login backend only accepts streaming requests, and Claude desktop's startup check (plus a background request on each model switch) is non-streaming, so it failed with `Invalid JSON response (HTTP 200)`. The gateway used by `relay-ai claude-app` and `relay-ai server` now streams behind the scenes for those models and returns one reply, as `relay-ai claude` already did. Chat itself was unaffected.
- **Claude desktop's catalog warning shows the real limit.** It said favorites were skipped beyond a 20-model limit; the limit is 50.
...
- **Command Code is now a builtin provider.** Add it with `relay-ai providers` → **+ Add a provider** → **Command Code** and paste your API key. No base URL to type, no second binary to run. Every Command Code plan except Go can call the Provider API. See [docs/PROVIDERS.md](docs/PROVIDERS.md) for the plan and model breakdown. ([Issue #66](https://github.com/jacob-bd/relay-ai/issues/66))
  - **Two protocols from one provider entry.** Command Code accepts Claude models only on its Anthropic-schema `/messages` endpoint and every other model only on its OpenAI-schema `/chat/completions` endpoint. Relay pins the correct protocol per model. This is why it ships as a builtin rather than a custom endpoint, which speaks one protocol or the other and cannot cover both.
  - **Only the models your plan can call are listed.** Command Code gates models by plan without following model family: on GOAT, `gpt-5.6-sol` works while `gpt-5.5` does not, and `gemini-3.8-flash` works while `gemini-3.5-flash` does not. No endpoint reports your plan, so Relay sends one minimal request per model when you add the provider or refresh its models, and drops the ones your plan rejects. Gated models are refused before they generate anything. Verified against a GOAT subscription: 50 models, matching Command Code's published GOAT roster exactly. Adding or refreshing takes roughly 20 to 30 seconds; refresh again after changing plans.
  - **A provider outage does not shrink your catalog.** Only an explicit `MODEL_NOT_IN_PLAN` rejection removes a model. Command Code's HTTP 503 for an overloaded upstream keeps it listed.
  - **The Go plan is not supported.** It works only through the private endpoint the Command Code CLI uses, which rejects requests that do not carry the CLI's own identifying headers. Reaching it would mean impersonating the CLI to bypass a control the vendor added deliberately, which also risks the user's account.

### Fixed

- **Gemini rejected every request in a Codex session.** Codex declares optional list arguments as a union type (`type: ["array", "null"]`), which the Google AI SDK converts into a schema whose `items` has no parent `type`. Gemini rejected the whole tool list with `field predicate failed: $type == Type.ARRAY` (HTTP 400), so plain text prompts failed exactly like image prompts did. Union types are now collapsed before the schema reaches Google and Vertex routes; other providers are untouched, since the union form is correct and required for strict OpenAI schemas. ([Issue #72](https://github.com/jacob-bd/relay-ai/issues/72))
- **OpenRouter returned HTTP 402 on trivial prompts.** Codex requests a blind 65,536 output tokens regardless of the model's real limit, and OpenRouter reserves credit for the full amount before generating — so accounts with usable credit were rejected with "requires more credits, or fewer max_tokens". Relay no longer forwards that cap to OpenRouter routes and lets the provider apply the model default. ([Issue #72](https://github.com/jacob-bd/relay-ai/issues/72))
- **Codex Sub-agents fell back to native as designed.** In mixed mode, any request Codex marked as a Sub-agent was rejected with HTTP 503 when no Relay Sub-agent model was configured — including Codex's own internal `codex-auto-review` agent, which failed on every turn. A marked Sub-agent now uses the configured Relay Sub-agent when there is one, and otherwise routes normally: native model IDs and Codex-internal aliases go to Codex, Relay model IDs go to Relay. ([Issue #72](https://github.com/jacob-bd/relay-ai/issues/72))
- **Route audit reported failed turns as successful.** The SDK reports most upstream failures as a stream event rather than an exception, so `codex-route-audit.jsonl` recorded `outcome: "ok"` for sessions that failed on every request — the opposite of what the log is for. Failed and aborted streams are now recorded as errors with the upstream status. ([Issue #72](https://github.com/jacob-bd/relay-ai/issues/72))
- **Contributors: `npm test` no longer reads or writes your real `~/.relay-ai`.** The repository had no `vitest.config.ts`, so any test that did not set `RELAY_AI_HOME` itself operated on the developer's live preferences and provider registry, rewriting `config.json` on every run. A new setup file points each test file at a throwaway app home and clears the legacy `OPENCODE_STARTER_HOME` override, and `tests/config-isolation.test.ts` fails if that ever regresses. This never affected installed copies of Relay, only people running the test suite from a clone.
