...
- **`sdk-adapter.ts`** — Anthropic `/v1/messages` ↔ SDK, one turn per request (Claude Code owns the tool loop). `translateRequest(body, npm)` builds the SDK call params (messages, tools, tool_choice, instructions) and folds inline `role:'system'` messages — Claude Code injects the skills list / system-reminders this way — into the instructions prompt so they aren't dropped. `streamAnthropicResponse` maps the SDK event `stream` to Anthropic SSE and aborts after 120 seconds without an event; `generateAnthropicResponse` handles non-streaming. `thought_signature` round-trips: encoded into the Anthropic `tool_use.id` as `{id}::ts::{signature}` and decoded back into `providerOptions.google.thoughtSignature` (Gemini puts the signature on the tool-call parts, captured at `tool-input-start`). The SDK handles Gemini's strict `thought_signature` echo-back correctly — the reason a hand-rolled Gemini-native path used to be required.
